...
boot_types = c('Rao-Wu-Yue-Beaumont', 'Preston', 'weighted')
fit_types = c('design', 'design', 'weighted')
...
cis = map2(
  .x = res,
  .y = betaHat,
  .f = get_cis_revise0505,
  smooth_for_ci = FALSE,
  smooth_for_variance = TRUE,
  nknots_fpca_min = round(0.75*L),
  nknots_min = round(0.75*L), 
    # we change knots here, but not change knots used in get_betahat functions
    # also, we should use the same smooth function to smooth true estimates
  L = L
)
...
saveRDS(stats, file = paste0("results/NI/case12_L1440_", sim_id, ".rds"))