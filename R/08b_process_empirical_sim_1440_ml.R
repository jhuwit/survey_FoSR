...
L = 1440
force = FALSE
if (!file.exists(here::here("results", "simulations", "all_empirical_sim_res_ml.rds")) || force) {
  settings = expand_grid(iter = 1:nsim,
                         weight_type = c("uniform", "nh_weights", "mims_weights", "combo_weights")) %>%
    mutate(rn = row_number())

  files = list.files(here::here("results", "simulations", "empirical_sim_1440_ml"), full.names = TRUE)
  res_df = map(.x = files,
  ...
  write_rds(res_df, here::here("results", "simulations", "all_empirical_sim_res_ml.rds"), compress = "xz")

} else {
  res_df = read_rds(here::here("results", "simulations", "all_empirical_sim_res_ml.rds"))
}
