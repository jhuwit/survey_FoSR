...
miss = setdiff(1:600, nums)

settings %>% filter(fold %in% miss & len != 1440) %>%
  pull(fold)

settings %>% filter(fold %in% miss & len != 1440 & In == 100 &
                      len == 50) %>%
  pull(fold)  %>%
  paste(collapse = ",")

settings %>% filter(fold %in% miss & len != 1440 &  !(In == 100 & len == 50)) %>%
  pull(fold)  %>%
  paste(collapse = ",")
...
x = read_rds(here::here("results/simulations/survey_sim/fold_412.rds"))
process_file = function(x){
  ...
  if(nrow(f) > 0) {
    f = read_rds(x) %>%
      ...
      select(id, MISE, starts_with("mean"), starts_with("cover"), starts_with("var"), boot_type, n, n_boot, fold) %>%
  ...
  all_res = map(.x = files, process_file)
  map(.x = files, .f = remove_empty)
  all_res_df =
    all_res %>%
    keep(is_tibble) %>%
    bind_rows()
  which(sapply(all_res, function(x) length(x) == 0))

  which(is.null(all_res))
  all_res_null =
    all_res %>%
    keep(is.null)

  missing=  which(sapply(all_res, function(x) length(x) == 0))
  ...
  ar = bind_rows(all_res_f, all_res_1440)
  ...
  write_rds(all_res, here::here("results", "simulations", "all_ssr.rds"))
} else {
...
all_res = read_rds(here::here("all_ssr.rds"))
