...
if (!file.exists(here::here("results", "application.rds")) || force ){
  steps_df_small =
    steps_df %>%
    ...
    ungroup() %>%
    pivot_wider(names_from = hr, values_from = value, names_prefix = "hour_") %>%
    mutate(race_cat =
  ...
  Y = as.matrix(data %>% select(starts_with("hour")))
