...
if(!file.exists(here::here("results", "inf_level_exploration.rds"))) {
  ...
  res = read_rds(here::here("results", "inf_level_exploration.rds"))
}
...
png(here::here("manuscript", "figures", "informativeness_correlations.png"), width = 14, height = 6, units = "in", res = 350)
p1
dev.off()

res |>
  group_by(inf_level) |>
  summarize(across(contains("c_"), list(mean = mean, median = median)))
...
res |>
  ...
  scale_fill_manual(values = c("#FFE099FF", "#F76D5EFF", "#A50021FF"),
                    name = "Informativeness", labels = c("Uniform", "Medium", "High")) +
  theme_light(base_size = 14) +
  labs(x = "", y = "Ratio", title = "Ratio of selection probabilities at different percentiles") +
