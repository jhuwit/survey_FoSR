...
outfile = here::here("results", "simulations", "survey_sim_ar1", paste0("fold_", sprintf("%03d", ifold), ".rds"))
...
partial_dir = here::here("results", "simulations", "survey_sim_ar1", paste0("fold_", sprintf("%03d", ifold), "_partials"))
...
if(temp$len == 1440) {
  ...
  lst = generate_superpopulation_ar1(
    family = temp$family,
    I = 10e6,
    L = temp$len,
    snr_b = temp$snr_b,
  ...
  for (iter in seq_len(nsim)) {
    ...
    x = try({
      ...
      data_smoothed = get_smoothed_y(data = data, L = temp$len)

      # get beta tilde: fit pointwise model at each pt along functional domain
      betaHat = map(
        .x = fit_types,
        .f = get_betatilde,
        data = data_smoothed,
      ...
      # beta hat: smooth -- can skip this part bc we already smoothed
