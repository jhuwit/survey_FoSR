...
# getremote survey_FoSR/results/simulations/survey_sim_1440/* results/simulations/survey_sim_1440/
...
if (!file.exists(here::here("results", "simulations", "survey_sim_res_1440.rds")) || force ){
  ...
  test_unique = all.equal(unique(all_res$fold), 1:120)
  ...
  write_rds(all_res, here::here("results", "simulations", "survey_sim_res_1440.rds"))
} else {
  all_res = read_rds(here::here("results", "simulations", "survey_sim_res_1440.rds"))
}
...
plot_mise = function(df, x_var, facet_x, facet_y, scales = "free_y", custom_labeller, xlab, tl = NULL, lnr = 1) {
  if(is.null(facet_x)) {
    p = df %>%
      ...
      labs(x = xlab, y = "log MISE", color = "Estimation Type", fill = "Estimation Type", shape = "Estimation Type") +
...
plot_se = function(df, x_var, facet_x, facet_y, scales = "free_y", custom_labeller, xlab, tl = NULL, lnr = 1) {
  if(is.null(facet_x)) {
    p = df %>%
      rename(xvar := !!sym(x_var), facet_yf := !!sym(facet_y)) %>%
      ggplot(aes(x = factor(xvar), y = mean_pw_se, color = boot_type, fill = boot_type)) +
      facet_grid(. ~ facet_yf, labeller = custom_labeller, scales = scales) +
      geom_boxplot(
        box.colour = "black",
        median.linewidth = 1,
