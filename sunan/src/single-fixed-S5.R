...
one.simeff_fixed <- function() {
  ...
  v <- c(rep(70,5), rep(50,5)); names(v) <- 1:10
  ...
  n <- as.vector(sapply(unique(stage1$g), function(gi) ifelse(gi < 500, 8, 6)))
  ...
  stage2$w1 = with(stage2, ifelse(strat <= 5, 100/70, 100/50))
  stage2$w2 <- with(stage2, ifelse(g < 500, 10 / 8, 10 / 6))
...
readr::write_rds(rval_fixed, 'results/measurement_raw/single-fixed-S5.rds')
readr::write_csv(average_results, 'results/metrix_summary/single-fixed-S5.csv')
