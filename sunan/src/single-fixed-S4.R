...
one.simeff_fixed <- function() {
  ...
  n <- as.vector(sapply(unique(stage1$g), function(gi) ifelse(gi < 500, 8, 2)))
  ...
  stage2$w2 <- with(stage2, ifelse(g < 500, 10 / 8, 10 / 2))
...
readr::write_rds(rval_fixed, 'results/measurement_raw/single-fixed-S4.rds')
readr::write_csv(average_results, 'results/metrix_summary/single-fixed-S4.csv')
