...
one.simeff_fixed <- function(pop) {
  ...
  in2 <- mapply(function(gi, ni) {
    ids <- unique(stage1$id[stage1$g == gi])  # 选出当前PSU的所有个体ID
    sampled_ids <- sample(ids, ni)            # 抽取ni个个体ID
    which(stage1$id %in% sampled_ids)         # 返回这些个体的所有观测行
  }, unique(stage1$g), n)

  # Combine the extracted results and retain the complete observation
...
readr::write_rds(rval_fixed, 'results/measurement_raw/single-random-S2.rds')
readr::write_csv(average_results, 'results/metrix_summary/single-random-S2.csv')
