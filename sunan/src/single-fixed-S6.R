...
one.simeff_fixed <- function() {
  ...
  v <- c(rep(30,5), rep(10,5)); names(v) <- 1:10
  ...
  n <- as.vector(sapply(unique(stage1$g), function(gi) ifelse(gi < 500, 3, 1)))
  ...
  stage2$w1 = with(stage2, ifelse(strat <= 5, 100/30, 100/10))
  stage2$w2 <- with(stage2, ifelse(g < 500, 10 / 3, 10 / 1))
  stage2$w <- stage2$w1 * stage2$w2

  stage2$n1 <- rep(100, nrow(stage2))
...
readr::write_rds(rval_fixed, 'results/measurement_raw/single-fixed-S6.rds')
readr::write_csv(average_results, 'results/metrix_summary/single-fixed-S6.csv')
