...
one.simeff_fixed <- function(pop) {
  ...
  m0 <- lme4::lmer(y ~ (1+x|id) + x + z,
                   data = pop)

  # First stage sampling
  # v <- rep(5, 10); names(v) <- 1:10
  v <- c(rep(7,5), rep(5,5)); names(v) <- 1:10
  ...
  n <- as.vector(sapply(unique(stage1$g), function(gi) ifelse(gi < 50, 12, 16)))
  ...
  # Generate weights
  # stage2$w1 = with(stage2, 10 / 4) #
  stage2$w1 = with(stage2, ifelse(strat <= 5, 10/7, 10/5))
  stage2$w2 <- with(stage2, ifelse(g < 50, 20/12, 20/16))
  stage2$w <- stage2$w1 * stage2$w2
  stage2$wid <- 1

  stage2$n1 <- rep(10, nrow(stage2))
  stage2$n2 <- rep(10, nrow(stage2))

  # Fit LMER
  m1 <- lme4:::lmer(y~(1+x|id)+x+z,data=stage2)
  ...
  m2 <- svylme::svy2lme(y ~ (1+x|id) + x + z,
                        design = des,
                        method = 'nested',
                        return.devfun=TRUE)
  m2.coef = m2$beta[2]
  ...
  nonparametric_bootstrap <- function(stage2, boot.num = 100) {
    ...
    #   m.tmp = suppressWarnings( svylme::svy2lme(y ~ (1+x|id) + x + z,
    #                                             method = 'nested',
    #                                             design= boot_des) )
    #   m.tmp$beta[2]
    # })
...
readr::write_rds(rval_fixed, 'results/measurement_raw/single-random-S5.rds')
readr::write_csv(average_results, 'results/metrix_summary/single-random-S5.csv')
