...
one.simeff_fixed <- function(pop) {
  ...
  v <- c(rep(6,5), rep(2,5)); names(v) <- 1:10
  ...
  n <- as.vector(sapply(unique(stage1$g), function(gi) ifelse(gi < 50, 4, 16)))
  ...
  stage2$w1 = with(stage2, ifelse(strat <= 5, 10/6, 10/2))
  stage2$w2 <- with(stage2, ifelse(g < 50, 20 / 4, 20 / 16))
...
readr::write_rds(rval_fixed, 'results/measurement_raw/single-random-S4.rds')
readr::write_csv(average_results, 'results/metrix_summary/single-random-S4.csv')
