...
one.simeff_fixed <- function(pop) {
  ...
  pop$u <- rnorm(2000, s = 0.7)[pop$id] # 个体层面随机效应
  ...
  v <- c(rep(3,5), rep(2,5)); names(v) <- 1:10
  ...
  n <- as.vector(sapply(unique(stage1$g), function(gi) ifelse(gi < 50, 2, 6)))
  ...
  stage2$w1 = with(stage2, ifelse(strat <= 5, 10/3, 10/2))
  stage2$w2 <- with(stage2, ifelse(g < 50, 20/2, 20/6))
...
readr::write_rds(rval_fixed, 'results/measurement_raw/single-random-S6.rds')
readr::write_csv(average_results, 'results/metrix_summary/single-random-S6.csv')
