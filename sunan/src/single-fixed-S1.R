...
one.simeff_fixed <- function() {
  ## Total 10 strata, each strata, has 100 PSUs, each PSU has 20 bodies.
  pop <- data.frame(
    x = rnorm(10000, s=1),
    z = rgamma(10000, 2),
    id = 1:10000,
    g = rep(1:1000, each = 10), # Total 1000 PSUs, 10 individuals each
    strat = rep(1:10, each = 1000) # 10 strata, 100 subjects per stratum
  )
  ...
  m0 <- lm(y ~ x + z, data = pop)

  # First stage sampling: draw 40 PSUs from each stratum
  v <- rep(40, 10); names(v) <- 1:10
  # v <- c(rep(60,5), rep(20,5)); names(v) <- 1:10
  psus <- survey::stratsample(rep(1:10, each = 100), v)
  # Draw the specified number of PSUs (Primary Sampling Units) from each stratum.
  # Generate a vector of length 1000 corresponding to 10 strata (strat 1 to 10), each containing 100 PSUs
  # rep(100, 10), indicates drawing 10 PSUs from each stratum
  # When rep(100, 10) is used, it is equivalent to selecting all since there's no secondary sampling of individuals
  stage1 <- subset(pop, g %in% psus)
  # In equal probability sampling, the total number of individuals within a PSU is assumed, so explicit definition isn't necessary.
  ...
  # Generate weights
  stage2$w1 = with(stage2, 100 / 40) # There are 100 PSUs in a stratum, sample 40
  # stage2$w1 = with(stage2, ifelse(strat <= 5, 100/60, 100/20))
  stage2$w2 <- with(stage2, ifelse(g < 500, 10 / 5, 10 / 5))  # Sample 5 individuals evenly
  stage2$w <- stage2$w1 * stage2$w2

  stage2$n1 <- rep(100, nrow(stage2)) # Total number of PSUs within each stratum
  stage2$n2 <- rep(10, nrow(stage2)) # Total number of individuals within a PSU
  ...
  des <- survey::svydesign(id = ~g+id,
                           strata = ~strat,
                           weights = ~w1+w2,
                           # fpc = ~n1 + n2, # Finite population correction for variance estimation
                           data = stage2,
                           nest = TRUE)
...
# 3. Store results of multiple simulations
sim.num <- 500
set.seed(209)
pb <- progress_bar$new(
...
# 4. Manually calculate confidence intervals based on SE
methods <- c("sample", "svy", "svy_rwyb", "svy_preston", "svy_np")
...
readr::write_rds(rval_fixed, 'results/measurement_raw/single-fixed-S1.rds')
readr::write_csv(average_results, 'results/metrix_summary/single-fixed-S1.csv')
